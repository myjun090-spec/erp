...
export default function Page() {
  ...
  useEffect(() => { fetch("/api/integration-logs?systemCode=swis").then(r => r.json()).then(j => { if (j.ok) setLogs(j.data.items); }).finally(() => setLoading(false)); }, []);
  ...
  return (
    <>
      <PageHeader eyebrow="정부시스템 연계" title="사회복지시설정보시스템" description="시설 현황 보고" />
      <Panel className="p-4 mb-4"><div className="flex items-center justify-between"><div><div className="font-semibold">사회복지시설정보시스템 연동 설정</div><div className="text-sm text-[color:var(--text-secondary)]">API 연동 상태 및 동기화 로그</div></div><button className="rounded-full border border-[color:var(--primary)] px-4 py-2 text-sm font-semibold text-[color:var(--primary)]">수동 동기화</button></div></Panel>

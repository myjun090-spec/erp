...
import { generateVendorCode } from "@/lib/document-numbers";
...
export default function VendorNewPage() {
  ...
  const [qualificationDraft, setQualificationDraft] = useState<QualificationDraft>(emptyQualificationDraft);
  ...
  const handleSaveQualification = () => {
    ...
    setQualifications((current) =>
      qualificationDraft.itemIndex === null
        ? [nextItem, ...current]
        : current.map((item, index) => (index === qualificationDraft.itemIndex ? nextItem : item)),
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      pushToast({
        title: "등록 실패",
        description: json.message || "공급업체를 저장하지 못했습니다.",
        tone: "warning",
      });
    } catch {
      pushToast({
  ...
  return (
    <>
      <PageHeader
        ...
        title="공급업체 등록"
        description="새로운 공급업체와 적격심사 인증 정보를 함께 등록합니다."
        actions={
          <>
            <Link
      ...
      <div className="mt-6">
        <DataTable
          title="인증"
          description="공급업체 적격심사와 인증 정보를 등록합니다."
          actions={
            <button
              type="button"
          ...
          emptyState={{
            title: "등록된 인증이 없습니다",
            description: "필요한 적격심사나 인증을 미리 등록할 수 있습니다.",
            action: (
              <button
                type="button"
                onClick={openNewQualificationDrawer}
      ...
      <ConfirmDialog
        ...
        title="인증을 삭제할까요?"
        description="선택한 인증 정보는 현재 등록안에서 제거됩니다."
        confirmLabel="삭제"
        tone="danger"

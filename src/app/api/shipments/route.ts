...
import { generateShipmentNo } from "@/lib/document-numbers";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const docs = await db.collection("logistics_shipments")
      .find(buildProjectFilter(projectId, {}, projectAccessScope.allowedProjectIds))
...
export async function POST(request: Request) {
  const auth = await requireApiActionPermission("shipment.create");
  if ("error" in auth) return auth.error;
  try {
    const db = await getMongoDb();
    const body = stripProtectedCreateFields(await request.json());
    const now = new Date().toISOString();
    const result = await db.collection("logistics_shipments").insertOne({
      shipmentNo: generateShipmentNo(),
      ...body,
      status: resolveStatus(body.status, "planned"),
      logisticsStatus: body.logisticsStatus ?? "preparing",

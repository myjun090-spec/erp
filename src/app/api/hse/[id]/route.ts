...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const doc = await db.collection("hse_incidents").findOne({ _id: new ObjectId(id) });
    if (!doc) return NextResponse.json({ ok: false, message: "HSE 사고를 찾을 수 없습니다." }, { status: 404 });
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireApiActionPermission("hse.archive");
  if ("error" in auth) return auth.error;
  try {
    const { id } = await params;
    if (!ObjectId.isValid(id)) return NextResponse.json({ ok: false, message: "ID 형식이 올바르지 않습니다." }, { status: 400 });
    const db = await getMongoDb();
    const result = await db.collection("hse_incidents").updateOne(
      { _id: new ObjectId(id) },
      { $set: { status: "archived", updatedAt: new Date().toISOString(), updatedBy: buildActorSnapshot(auth.profile) } },
    );
    if (result.matchedCount === 0) return NextResponse.json({ ok: false, message: "HSE 사고를 찾을 수 없습니다." }, { status: 404 });
    return NextResponse.json({ ok: true });
  } catch (e) { return NextResponse.json({ ok: false, message: e instanceof Error ? e.message : "Unknown" }, { status: 500 }); }
}

export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireApiActionPermission("hse.update");
  if ("error" in auth) return auth.error;
  try {
    const { id } = await params;
    if (!ObjectId.isValid(id)) return NextResponse.json({ ok: false, message: "ID 형식이 올바르지 않습니다." }, { status: 400 });
    const body = await request.json() as Record<string, unknown>;
    const allowed = ["incidentType", "severity", "title", "occurredAt", "description", "status", "projectSnapshot", "siteSnapshot", "correctiveActions"];
    ...
    if ("ownerName" in body || "ownerOrgUnit" in body || "ownerPhone" in body || "ownerEmail" in body) {
      fields.ownerUserSnapshot = {
        displayName: (body.ownerName as string) || "",
        orgUnitName: (body.ownerOrgUnit as string) || "",
    ...
    const db = await getMongoDb();
    const result = await db.collection("hse_incidents").updateOne({ _id: new ObjectId(id) }, { $set: fields });
    if (result.matchedCount === 0) return NextResponse.json({ ok: false, message: "HSE 사고를 찾을 수 없습니다." }, { status: 404 });

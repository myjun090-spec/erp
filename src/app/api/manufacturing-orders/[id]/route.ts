...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const doc = await db.collection("manufacturing_orders").findOne({ _id: new ObjectId(id) });
    if (!doc) return NextResponse.json({ ok: false, message: "제작지시를 찾을 수 없습니다." }, { status: 404 });
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireApiActionPermission("manufacturing-order.update");
  if ("error" in auth) return auth.error;
  try {
    const { id } = await params;
    ...
    const allowed = ["plannedStartDate", "plannedEndDate", "status", "vendorSnapshot", "quantity"];
    ...
    const result = await db.collection("manufacturing_orders").updateOne({ _id: new ObjectId(id) }, { $set: updateFields });
    if (result.matchedCount === 0) return NextResponse.json({ ok: false, message: "제작지시를 찾을 수 없습니다." }, { status: 404 });

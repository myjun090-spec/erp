...
export async function POST(request: Request) {
  try {
    ...
    const auth = await requireApiActionPermission("vendor.update");
    if ("error" in auth) return auth.error;
    const objectIds = targetIds.map((id) => new ObjectId(id));
    const now = new Date().toISOString();
    let updateFields: Record<string, unknown> = {};
    switch (action) {
      case "request-approval": updateFields = { status: "approval-pending", updatedAt: now }; break;
      case "suspend": updateFields = { status: "suspended", updatedAt: now }; break;
      case "restore": updateFields = { status: "active", updatedAt: now }; break;
      default: return NextResponse.json({ ok: false, message: `Unknown action: ${action}` }, { status: 400 });
    }
    const result = await db.collection("parties").updateMany({ _id: { $in: objectIds }, partyRoles: "vendor" }, { $set: updateFields });

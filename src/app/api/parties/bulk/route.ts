...
export async function POST(request: Request) {
  try {
    ...
    const auth = await requireApiActionPermission(
      action === "archive" || action === "restore" ? "party.archive" : "party.update",
    );
    ...
    switch (action) {
      case "activate":
        updateFields = { status: "active", updatedAt: now };
        break;
      case "suspend":
        updateFields = { status: "suspended", updatedAt: now };
        break;
      case "archive":
        updateFields = { status: "archived", updatedAt: now };
        break;
      case "restore":
        updateFields = { status: "active", updatedAt: now };
        break;
    ...
    const result = await db.collection("parties").updateMany(
      { _id: { $in: objectIds } },
      { $set: updateFields },
    );

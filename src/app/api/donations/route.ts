...
export async function GET(request: Request) {
  ...
  try {
    ...
    const docs = await db.collection("donations").find(filter).sort({ updatedAt: -1 }).limit(200).toArray();
    const items = docs.map((doc) => ({
      ...
      receiptIssued: doc.receiptIssued,
      status: doc.status,
    }));
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const counter = await db.collection("counters").findOneAndUpdate(
      { _id: "donations" as any }, { $inc: { seq: 1 } }, { upsert: true, returnDocument: "after" }
    );
    const seq = String(counter?.seq ?? 1).padStart(4, "0");
    const docNo = `DON-${now.slice(0,4)}${now.slice(5,7)}-${seq}`;
    const donationNo = body.donationNo ?? null;
    const donorSnapshot = body.donorSnapshot ?? null;
    const donationType = body.donationType ?? null;
    const amount = body.amount ?? null;
    const paymentMethod = body.paymentMethod ?? null;
    const receiptIssued = body.receiptIssued ?? null;

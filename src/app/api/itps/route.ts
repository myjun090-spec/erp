...
export async function POST(request: Request) {
  const auth = await requireApiActionPermission("itp.create");
  ...
  try { const db = await getMongoDb(); const body = stripProtectedCreateFields(await request.json()); const now = new Date().toISOString();
    const result = await db.collection("itps").insertOne({ ...body, status: resolveStatus(body.status, "active"), approvalStatus: resolveStatus(body.approvalStatus, "draft"), ...buildCreateMetadata(auth.profile, now) });

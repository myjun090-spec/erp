...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const doc = await db.collection("ncrs").findOne({ _id: new ObjectId(id) });
    if (!doc) return NextResponse.json({ ok: false, message: "NCR을 찾을 수 없습니다." }, { status: 404 });
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireApiActionPermission("ncr.update");
  if ("error" in auth) return auth.error;
  try {
    const { id } = await params;
    if (!ObjectId.isValid(id)) return NextResponse.json({ ok: false, message: "ID 형식이 올바르지 않습니다." }, { status: 400 });
    const body = await request.json() as Record<string, unknown>;
    const allowed = ["severity", "disposition", "description", "dueDate", "status", "projectSnapshot", "systemSnapshot", "ownerUserSnapshot"];
    const fields: Record<string, unknown> = {};
    for (const key of allowed) { if (key in body) fields[key] = body[key]; }
    fields.updatedAt = new Date().toISOString();
    fields.updatedBy = buildActorSnapshot(auth.profile);
    const db = await getMongoDb();
    const result = await db.collection("ncrs").updateOne({ _id: new ObjectId(id) }, { $set: fields });
    if (result.matchedCount === 0) return NextResponse.json({ ok: false, message: "NCR을 찾을 수 없습니다." }, { status: 404 });
    return NextResponse.json({ ok: true, affectedCount: result.modifiedCount });
  } catch (e) { return NextResponse.json({ ok: false, message: e instanceof Error ? e.message : "Unknown" }, { status: 500 }); }
}

export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireApiActionPermission("ncr.archive");
  if ("error" in auth) return auth.error;
  try {
    const { id } = await params;
    if (!ObjectId.isValid(id)) return NextResponse.json({ ok: false, message: "ID 형식이 올바르지 않습니다." }, { status: 400 });
    const db = await getMongoDb();
    const result = await db.collection("ncrs").updateOne(
      { _id: new ObjectId(id) },

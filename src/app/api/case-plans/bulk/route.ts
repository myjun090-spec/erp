...
const actionPermissionMap: Record<string, KnownPermissionCode> = {
  submit: "case-plan.update",
  approve: "case-plan.approve",
  reject: "case-plan.reject",
  complete: "case-plan.update",
  terminate: "case-plan.update",
};
...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(targetIds) || targetIds.length === 0) {
      return NextResponse.json({ ok: false, message: "대상을 선택해 주세요." }, { status: 400 });
    }

    const objectIds = targetIds.filter((id) => ObjectId.isValid(id)).map((id) => new ObjectId(id));
    if (objectIds.length === 0) {
      return NextResponse.json({ ok: false, message: "유효한 대상이 없습니다." }, { status: 400 });
    }
    ...
    if (action === "approve") {
      setFields.approverSnapshot = actorSnapshot;
    }

    const result = await db.collection("case_plans").updateMany(
      { _id: { $in: objectIds } },
      {
        $set: setFields,

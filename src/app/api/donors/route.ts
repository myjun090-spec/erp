...
export async function GET(request: Request) {
  ...
  try {
    ...
    const docs = await db.collection("donors").find(filter).sort({ updatedAt: -1 }).limit(200).toArray();
    const items = docs.map((doc) => ({
      ...
      name: doc.name,
      taxIdNo: doc.taxIdNo,
      donorCategory: doc.donorCategory,
      status: doc.status,
    }));
...
export async function POST(request: Request) {
  const auth = await requireApiActionPermission("donor.create");
  if ("error" in auth) return auth.error;
  try {
    const db = await getMongoDb();
    ...
    const counter = await db.collection("counters").findOneAndUpdate(
      { _id: "donors" as any }, { $inc: { seq: 1 } }, { upsert: true, returnDocument: "after" }
    );
    const seq = String(counter?.seq ?? 1).padStart(4, "0");
    const docNo = `DNR-${now.slice(0,4)}${now.slice(5,7)}-${seq}`;
    const donorNo = body.donorNo ?? null;
    const donorType = body.donorType ?? null;
    const name = body.name ?? null;
    const taxIdNo = body.taxIdNo ?? null;
    const donorCategory = body.donorCategory ?? null;
